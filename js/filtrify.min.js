...
(function ($, window, document, undefined) {
  ...
  Filtrify.prototype.load = function () {
    ...
    this._items.each(
      this._bind(function (index, element) {
        ...
        for (i = 0; i < attr.length; i++) {
          ...
          if (
            name.indexOf("data-") === 0 &&
            $.inArray(name, this.options.block) === -1
          ) {
            ...
            for (t = 0; t < tags.length; t++) {
              if (tags[t].length) {
                ...
                this._fields[field][tags[t]] =
                  this._fields[field][tags[t]] === undefined
                    ? 1
                    : this._fields[field][tags[t]] + 1;
              }
            }
          }
        }
    ...
    for (f; f < this._order.length; f++) {
      field =
        browser.webkit || browser.opera
          ? this._order[f]
          : this._order[this._order.length - f - 1];
      this._menu[field] = {};
      this.build(field);
      this.cache(field);
      this.events(field);
  ...
  Filtrify.prototype.preventOverflow = function (f) {
    ...
    high_top =
      this._menu[f].highlight.position().top + this._menu[f].tags.scrollTop();
    high_bottom = high_top + this._menu[f].highlight.outerHeight();
    if (high_bottom >= visible_bottom) {
      return this._menu[f].tags.scrollTop(
  ...
  Filtrify.prototype.rewriteFields = function () {
    var field;
    for (field in this._fields) {
      this._menu[field].tags.children().each(
        this._bind(function (index, element) {
          var tag = element.textContent || element.innerText,
            count =
              this._fields[field][tag] === undefined
                ? 0
                : this._fields[field][tag];
  ...
  Filtrify.prototype.showMatch = function (row, match) {
    if (this.options.hide) {
      ...
      if (match) {
        if (hidden)
          this._items[row].className = this._items[row].className.replace(
            /ft-hidden/g,
            ""
          );
      } else {
        if (!hidden)
          this._items[row].className =
            this._items[row].className + " ft-hidden";
      }
    }
  };
  ...
  Filtrify.prototype.updatePanel = function (f) {
    ...
    for (t; t < this._query[f].length; t++) {
      tag = tags.filter(
        this._bind(function (index) {
          return (
            (tags[index].textContent || tags[index].innerText) ===
            this._query[f][t]
          );
        }, this)
      );
